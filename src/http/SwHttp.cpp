...
bool SwHttp::HttpPost(const std::string& url, const std::string& dat, std::string& resp) const
{
	//printf("data:[%s]\n", dat.c_str());
	long rc = 0;
	try
...
void SwHttp::SetEasyOpt(const std::string& url, void* curl, std::string* resp) const
{
	// set opts  
...
size_t SwHttp::WriteCallback(void* data, size_t size, size_t nmemb, void* buffer)
{
